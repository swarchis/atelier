...
export default function TechPackDetail() {
  ...
  // --- Handlers: BOM ---
  const updateBom = (rowId, field, value) => {
    setBom(prev => prev.map(item => item.id === rowId ? { ...item, [field]: value } : item));
  };
  const addBomRow = () => {
    setBom(prev => [...prev, { id: `bom-${Date.now()}`, material: '', supplier: '', qtyPerUnit: '', unitCost: '' }]);
  };
  const removeBomRow = (rowId) => {
    setBom(prev => prev.filter(item => item.id !== rowId));
  };

  // --- Handlers: Measurements ---
  const updateMeas = (rowId, field, value) => {
    setMeasurements(prev => prev.map(item => item.id === rowId ? { ...item, [field]: value } : item));
  };
  const addMeasRow = () => {
    setMeasurements(prev => [...prev, { id: `meas-${Date.now()}`, size: '', chest: '', length: '', sleeve: '' }]);
  };
  const removeMeasRow = (rowId) => {
    setMeasurements(prev => prev.filter(item => item.id !== rowId));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        }, { onConflict: 'product_id' }); // Upsert uses the unique product_id

      if (error) throw error;
      
      // Temporary native alert just to show success for the MVP
      ...
      <div className="content">
        {loadingData ? (
          ...
          <>
            ...
            {/* Other tabs omitted for brevity but remain the same logic as before */}
            {tab === 'sampling' && (
              <EmptyState icon="ph-scissors" title="No samples logged" color="var(--c-materials)" sub="Track proto → fit → revised → size set → pre-production here." />
            )}

            {tab === 'readiness' && (
              <EmptyState icon="ph-check-circle" color="var(--c-finalcheck)" title="No readiness checklist yet" sub="The final pre-production validation checklist appears here." />
            )}
          </>
        )}
      </div>
